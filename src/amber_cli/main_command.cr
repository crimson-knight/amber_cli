...
module AmberCLI
  class MainCommand < AmberCLI::Core::BaseCommand
    ...
    def setup_command_options
      ...
      option_parser.separator "  target add android    Add Android without replacing the existing web app"
      option_parser.separator "  doctor|build android  Inspect or build the Android target"
      option_parser.separator "  run|test android      Run or test with an explicit --device SERIAL"
